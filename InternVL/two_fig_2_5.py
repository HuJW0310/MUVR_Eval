...
# video_path = '/mnt/wfs/mmchongqingssdwfssz/project_wx-search-alg-gs/sleepfeng/VLLM_Eval/LLaVA-NeXT/playground/demo/xU25MMA2N4aVtYay.mp4'
...
class VideoQAProcessor:
    ...
    def build_video_prefix(num_frames, base=0):
        if num_frames == 1:
            return "".join([f"Image-{i+1+base}: <image>\n" for i in range(num_frames)])
    ...
    def process_video(self, query_path, target_path, item, is_first=True):
        # Load video frames
        pixel_values_q, num_patches_list_q = load_video(query_path, num_segments=1, max_num=1)
        pixel_values_q = pixel_values_q.to(torch.bfloat16).cuda()
        ...
        pixel_values = torch.cat((pixel_values_q, pixel_values_t), dim=0)
        
        # Build question
        # item['Tag'] = ''
        video_prefix_q = self.build_video_prefix(len(num_patches_list_q), base=0)
        video_prefix_t = self.build_video_prefix(len(num_patches_list_t), base=len(num_patches_list_q))
        # Let's think step by step.
        # if item['Tag'] != "":
        #     question = f"I will give you two videos or images: [Query] and [Target]. Please decide whether [Target] should be retrieved according to its relevance ({item['split_prompt']}) to [Query]. I will further prompt you [Focus] to help you focus on what i am interested in the [Query]. I will also provide [Tag]. If [Tag] starts with '+', [Target] (if retrieved) must feature the [Tag]. If [Tag] starts with '-', [Target] (if retrieved) must not feature the [Tag].\n[Query]:\n" + video_prefix_q + "\n[Target]:\n" + video_prefix_t + "\n[Focus]:\n" + item['prompt_en'] + "\n[Tag]:\n" + item['Tag'] +"\n[Output]:\nIf retrieve, return Yes. If not, return No."
        # else:
        #     question = f"I will give you two videos or images: [Query] and [Target]. Please decide whether [Target] should be retrieved according to its relevance ({item['split_prompt']}) to [Query]. I will further prompt you [Focus] to help you focus on what i am interested in the [Query].\n[Query]:\n" + video_prefix_q + "\n[Target]:\n" + video_prefix_t + "\n[Focus]:\n" + item['prompt_en'] + "\n[Output]:\nIf retrieve, return Yes. If not, return No."
        # print(item)
        if item['Tag'] != "":
            question = f"I will give you a video query and a video target: [Query] and [Target]. Please determine whether any part of [Target] is slightly relevant to any part of [Query] or [Focus]. I will also provide [Tag] that [Target] (if relevant) must feature it.\n[Query]:\n" + video_prefix_q + "\n[Target]:\n" + video_prefix_t + "\n[Focus]\n" + item['prompt_en'] + "\n[Tag]:\n" + item['Tag'] +"\n[Output]:\nIf slightly relevant, return Yes. If not, return No."
        else:
            question = f"I will give you a video query and a video target: [Query] and [Target]. Please determine whether any part of [Target] is slightly relevant to any part of [Query] or [Focus].\n[Query]:\n" + video_prefix_q + "\n[Target]:\n" + video_prefix_t + "\n[Focus]\n" + item['prompt_en'] + "\n[Output]:\nIf slightly relevant, return Yes. If not, return No."
        ...
        response, self.history = self.model.chat(
            ...
            question,
            self.gen_config,
            num_patches_list=num_patches_list_q+num_patches_list_t,
...
class JSONLDataset(Dataset):
    def __init__(self, jsonl_path, video_folder):
        ...
        if 'all' in jsonl_path.name:
            ...
            splits = ['news', 'animal', 'geng', 'region', 'dance']
...
def main():
    ...
    split = 'all'
    parser.add_argument("--video_folder", type=str, default=f'/mnt/wfs/mmchongqingssdwfssz/project_wx-search-alg-gs/sleepfeng/datasets/VR/M2VR/video_6_336/all_file/{split}')
    parser.add_argument("--jsonl_path", type=str, default=f'/mnt/wfs/mmchongqingssdwfssz/project_wx-search-alg-gs/sleepfeng/s2vs/WebVR_Rerank/{split}_20.jsonl')
    # parser.add_argument("--jsonl_path", type=str, default=f'/mnt/wfs/mmchongqingssdwfssz/project_wx-search-alg-gs/sleepfeng/s2vs/WebVR_Rerank/{split}_tv_0.05_clip.jsonl')
